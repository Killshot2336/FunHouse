-- Full game reset for production (all housemates)
...
-- Household activity (fresh week for showing dad)
...
DELETE FROM mood_checkins WHERE user_id IN ('aden', 'edward', 'jamie');
DELETE FROM mini_game_state WHERE user_id IN ('aden', 'edward', 'jamie');
